	...
	if(!ctype_alnum($name)){
		...
		if($newname == ''){
			...
			if ($action == "0") {
				...
				file_put_contents($path."/usage_log.txt",$track,FILE_APPEND);
				...
				file_put_contents($path."/usage_log.txt",$track1,FILE_APPEND);
				//track 2
				$track2 = $_SERVER['REMOTE_ADDR']."|".$time."|".$newname."|user|new name\n";
				file_put_contents($path."/usage_log.txt",$track2,FILE_APPEND);
